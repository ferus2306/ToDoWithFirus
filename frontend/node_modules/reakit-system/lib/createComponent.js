...
function createComponent(_ref) {
  ...
  var Comp = function Comp(_ref2, ref) {
    ...
    if (useHook) {
      ...
      var allProps = // If `as` is a string, then that means it's an element not a component
      ...
      reakitUtils.isPlainObject(props.state) && typeof as !== "string" ? _rollupPluginBabelHelpers._objectSpread2({
        state: props.state
      }, elementProps) : asOptions ? _rollupPluginBabelHelpers._objectSpread2(_rollupPluginBabelHelpers._objectSpread2({}, elementProps), asOptions) : elementProps;
  ...
  if (shouldMemo) {
    Comp = memo(Comp, propsAreEqual && reakitUtils.normalizePropsAreEqual(propsAreEqual));
  }

  Comp.__keys = keys;
  Comp.unstable_propsAreEqual = reakitUtils.normalizePropsAreEqual(propsAreEqual || shallowEqual.shallowEqual);
  return Comp;
}
